module "vault_hvd_primary" {
  source = "git@github.com:hashicorp/terraform-aws-vault-enterprise-hvd?ref=hotfix/abc-vault-ssm-base64-support"
  #------------------------------------------------------------------------------
  # Common
  #------------------------------------------------------------------------------
  friendly_name_prefix = "vaultabc"
...
module "vault_hvd_public_private" {
  ...
  # Common
  #------------------------------------------------------------------------------
  friendly_name_prefix = "vaultabc-pub-priv"
  ...
  #------------------------------------------------------------------------------
  sm_vault_license_arn      = aws_secretsmanager_secret.vault_license.arn
  sm_vault_tls_cert_arn     = aws_secretsmanager_secret.vault_tls_cert_b64.arn
  sm_vault_tls_cert_key_arn = aws_secretsmanager_secret.vault_tls_privkey_b64.arn
  sm_vault_tls_ca_bundle    = null # publicly trusted cert from Let's Encrypt, so no CA bundle
  vault_seal_awskms_key_arn = aws_kms_key.unseal.arn
  ...
  depends_on = [
    aws_secretsmanager_secret_version.vault_license,
    aws_secretsmanager_secret_version.vault_tls_cert_b64,
    aws_secretsmanager_secret_version.vault_tls_privkey_b64,
    aws_kms_key.unseal
  ]
}
...
#   # Common
#   #------------------------------------------------------------------------------
#   friendly_name_prefix = "vaultabc-pub-pub"
